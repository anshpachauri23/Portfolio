...
public class Program2 extends ProgramSecondary {
    ...
    public final Map<String, Statement> newContext() {
        ...
        // Return the variable of the same type of context.
    ...
    public final void swapContext(Map<String, Statement> c) {
        ...
        assert noPrimitiveInstructions(
                c) : "Violation of: names in c do not match the names"
                        + " of primitive instructions in the BL language";
        assert allBlocks(c) : "Violation of: bodies in c"
                + " are all BLOCK statements";

        // Create a temporary Map to store the current context
        Map<String, Statement> temp = this.context.newInstance();

        // Transfer the contents of this.context to the temp Map
        temp.transferFrom(this.context);

        // Transfer the contents of Map c to this.context,
        // effectively swapping their contents
        this.context.transferFrom(c);

        // Transfer the contents of the temporary temp Map back to Map c
        c.transferFrom(temp);
    }
    ...
    public final Statement newBody() {
        ...
        // Returns new body for the Statement
    ...
    public final void swapBody(Statement b) {
        ...
        // Create a temporary Statement to store the current body
        Statement temp = this.body.newInstance();

        // Transfer the contents of this.body to the temp Statement
        temp.transferFrom(this.body);

        // Transfer the contents of Statement b to this.body,
        // effectively swapping their contents
        this.body.transferFrom(b);

        // Transfer the contents of the temporary temp Statement back to Statement b
        b.transferFrom(temp);
    }
