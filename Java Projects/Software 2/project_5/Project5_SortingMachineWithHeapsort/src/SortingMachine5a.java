...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (left <= last) {
            if (right <= last) {
                ...
                if (order.compare(array[top], array[smallest]) > 0
                        && smallest == left) {
                    exchangeEntries(array, top, left);
                    siftDown(array, left, last, order);
                } else if (order.compare(array[top], array[smallest]) > 0
                        && smallest == right) {
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // Copy elements from the queue to the heap array
        int index = 0;
        while (q.length() > 0) {
            T element = q.dequeue();
            heap[index] = element;
        ...
        // Build the heap by ensuring the heap property
        for (int i = (heap.length - 1) / 2; i >= 0; i--) {
            heapify(heap, i, order);
        }
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        // Set up insertion mode according to contract
        this.insertionMode = true;
        // Set up comparator
        this.machineOrder = order;
        // Create representation variable
        this.entries = new Queue1L<T>();
        // Set the number of elements in a queue
        this.heapSize = 0;
        // Build heap
        this.heap = buildHeap(this.entries, order);
    ...
    public final T removeFirst() {
        ...
        T x = this.heap[0];
        // switch root with last entry in heap
