...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        // Determine the bucket where the key-value pair should be stored
        int bucket = mod(hashKey, this.hashTable.length);
        // Add the key and value to the appropriate bucket in the hash table
    ...
    public final Pair<K, V> remove(K key) {
        ...
        // Calculate the bucket where the key should be located
        int bucket = mod(hashKey, this.hashTable.length);

        // Remove the key-value pair from the hashTable's bucket
        Pair<K, V> temp = this.hashTable[bucket].remove(key);

        // Decrement the size of the hashTable as a key-value pair was removed
        this.size--;
    ...
    public final Pair<K, V> removeAny() {
        ...
        // Find a non-empty bucket in the hash table
        int i = 0;
        while (this.hashTable[i].size() == 0) {
            i++;
        }

        // Remove an arbitrary entry from the selected bucket
        Pair<K, V> removedPair = this.hashTable[i].removeAny();
    ...
    public final V value(K key) {
        ...
        // Ensure that the key is present in the hash table
        assert this.hasKey(key) : "Violation of: key is in DOMAIN(this)";

        // Initialize variables for the hashed key and bucket index
        int hashKey = key.hashCode();
        int bucket = mod(hashKey, this.hashTable.length);

        // Retrieve the value associated with the key from the hash table
        V val = this.hashTable[bucket].value(key);
    ...
    public final boolean hasKey(K key) {
        ...
        // Determine the bucket index using the modulo operation
        int bucket = mod(hashKey, this.hashTable.length);

        // Check if the hash table's bucket at the calculated index contains the key
        boolean result = this.hashTable[bucket].hasKey(key);

        // Return the result indicating whether the key is present in the hash table
        return result;
    }
