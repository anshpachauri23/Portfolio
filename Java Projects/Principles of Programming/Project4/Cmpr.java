public class Cmpr {
    ...
    // Evaluates the comparison and returns the boolean result.
    boolean evaluate(MemoryManager memory) {
        int leftValue = leftExpr.evaluate(memory);
        int rightValue = rightExpr.evaluate(memory);
        ...
        switch (operator) {
            ...
            case LESS:
                return leftValue < rightValue;
            default:
                System.out.println("ERROR: Invalid comparison operator: " + operator);
                System.exit(1);
