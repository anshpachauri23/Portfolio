...
class NaiveBayesClassifier(object):
    ...
    def train(self, training_sentences, training_labels):
        ...
        # Use the tokenized training_set instead of raw training_sentences.
        # training_set was obtained earlier via word_tokenization_dataset.
        self.V = {}
        index = 0
        for sentence in training_set:   # <-- use training_set (tokenized)!
            for word in sentence:
                if word not in self.V:
                    self.V[word] = index
                    index += 1

        # Create a binary bag-of-words matrix X.
        # Each row corresponds to a sentence represented as a D-dimensional binary vector,
        # where D is the size of self.V.
        D = len(self.V)
        N = len(training_set)
        X = np.zeros((N, D), dtype=int)
        for i, sentence in enumerate(training_set):   # <-- use training_set here too!
            for word in set(sentence):  # Ensure each word is counted only once per sentence.
                j = self.V[word]
                X[i, j] = 1

        label_map = {-1: 0, 0: 1, 1: 2}  # Map each label to a row index (0, 1, 2)
        N, D = X.shape  # X is the binary bag-of-words matrix
        self.prior = np.zeros(3)
        self.conditional = np.zeros((3, D))
        for c in label_map:
            # Find indices in X corresponding to class c.
            indices = [i for i in range(N) if training_labels[i] == c]
            num_c = len(indices)
            self.prior[label_map[c]] = num_c / N
            sum_X_c = np.sum(X[indices, :], axis=0)
            self.conditional[label_map[c], :] = sum_X_c / num_c
        # -------- TO DO (end) --------#
    ...
    def predict(self, test_sentence):
        ...
        # Convert the tokenized test_sentence (already tokenized above) into a binary BoW vector.
        D = len(self.V)
        x_test = np.zeros(D, dtype=int)
        for word in set(test_sentence):
            if word in self.V:
                x_test[self.V[word]] = 1

        # Compute the log probability for each class.
        for c in [-1, 0, 1]:
            # Map class c to the corresponding index.
            if c == -1:
                idx = 0
            elif c == 0:
                idx = 1
            ...
            log_likelihood = 0.0
            for d in range(D):
                p_cd = self.conditional[idx, d]  # p(x[d] = 1 | Y = c)
                if x_test[d] == 1:
                    # If word is present, add log probability.
                    if p_cd == 0:
                        log_likelihood += -np.inf 
                    else:
                        log_likelihood += np.log(p_cd)
                else:
                    # If word is absent, add log(1 - probability).
                    if p_cd == 1:
                        log_likelihood += -np.inf
                    else:
                        log_likelihood += np.log(1 - p_cd)
            label_probability[c] = log_prior + log_likelihood
        # -------- TO DO (end) --------#
